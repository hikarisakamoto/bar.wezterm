...
---appends a trailing slash to a directory path, unless it already carries one.
---anything that is not a non-empty string is returned untouched.
---@param path any
---@return any
M._with_trailing_slash = function(path)
  if type(path) ~= "string" or #path == 0 or path:sub(-1) == "/" then
    return path
  end
  return path .. "/"
end
...
M.resolve = function(rule_list, context)
  ...
  for _, rule in ipairs(rule_list) do
    ...
    for _, field in ipairs(M.matchable) do
      local matcher = rule[field]
      if matcher ~= nil then
        local value = ctx[field]

        -- a working directory is compared as a directory: both sides carry a
        -- trailing slash, so a rule anchored on a directory boundary matches
        -- that directory as well as everything beneath it, and never a sibling
        -- that merely shares its prefix
        if field == "cwd" then
          value = M._with_trailing_slash(value)
          if type(matcher) == "string" then
            matcher = M._with_trailing_slash(matcher)
          end
        end

        if not M.matches(matcher, value) then
          goto continue
        end
        tested = true
      end
